...
// Normalises the stored stats into an editable row list { label, value }.
// Handles both the new `{ rows }` shape and the older
// `{ countries, tourPackages }` shape saved by earlier admin versions, so
// both old and new saves render identically on the site.
export function normalizeStats(stats) {
  if (Array.isArray(stats?.rows) && stats.rows.length) {
  ...
  const rows = [];
  if (stats && stats.countries != null) {
    rows.push({ label: stats.labelCountries || "Countries", value: Number(stats.countries) || 0 });
  }
  if (stats && stats.tourPackages != null) {
    rows.push({ label: stats.labelTourPackages || "Tour Packages", value: Number(stats.tourPackages) || 0 });
  }
  if (rows.length) return rows;
  return [
    { label: "Countries", value: 4 },
    { label: "Tour Packages", value: 12 },
  ];
}
