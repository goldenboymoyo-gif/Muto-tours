...
// The backend is a free Render instance that cold-starts in 10–60s, so the
// default fetch is intentionally generous rather than snappy.
export async function fetchStoredContent() {
  try {
    const res = await fetchWithTimeout(`${API_URL}/api/content`, { timeout: 15000 });
